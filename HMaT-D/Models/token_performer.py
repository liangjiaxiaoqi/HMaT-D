...
class Token_performer_HMSA_decoer_2(nn.Module):
    def __init__(self, dim, in_dim, head_cnt=8, kernel_ratio=0.5, dp1=0.1, dp2=0.1):
        super(Token_performer_HMSA_decoer_2, self).__init__()
        self.in_dim = in_dim
        ...
        self.kqv = nn.Linear(dim, 3 * self.emb)
        self.dp = nn.Dropout(dp1)
        self.heads_to_one_head = nn.Linear(self.emb, self.in_dim)
        self.norm1 = nn.LayerNorm(dim)
        self.norm2 = nn.LayerNorm(self.in_dim)
        self.epsilon = 1e-8

        self.mlp = nn.Sequential(
            nn.Linear(self.in_dim, 1 * self.in_dim),
            nn.GELU(),
            nn.Linear(1 * self.in_dim, self.in_dim),
            nn.Dropout(dp2),
        )

        self.m = int(self.in_dim * kernel_ratio)
        self.w = torch.randn(self.m, self.in_dim)
        self.w = nn.Parameter(nn.init.orthogonal_(self.w) * math.sqrt(self.m), requires_grad=False)
    ...
    def multi_head_attn_1(self, x):
        kqv = self.kqv(x)
        k, q, v = torch.split(kqv, self.emb, dim=-1)
    ...
    def multi_head_attn(self, x):
        kqv_output = self.kqv(x)
        k, q, v = torch.split(kqv_output, self.emb, dim=-1)

        k_list = k.chunk(self.head_cnt, dim=-1)
        q_list = q.chunk(self.head_cnt, dim=-1)
        v_list = v.chunk(self.head_cnt, dim=-1)

        attn_outputs = []
        for k_head, q_head, v_head in zip(k_list, q_list, v_list):
            kp, qp = self.prm_exp(k_head), self.prm_exp(q_head)
            D = torch.einsum('bti,bi->bt', qp, kp.sum(dim=1)).unsqueeze(dim=2)
            kptv = torch.einsum('bin,bim->bnm', v_head.float(), kp)
            y = torch.einsum('bti,bni->btn', qp, kptv) / (D.repeat(1, 1, self.in_dim) + self.epsilon)
            attn_outputs.append(y)

        attn_output = torch.cat(attn_outputs, dim=-1)
        attn_output = self.dp(self.heads_to_one_head(attn_output))
        return attn_output

    def forward(self, x):
        x = x + self.multi_head_attn(self.norm1(x))
        x = x + self.mlp(self.norm2(x))
        return x
