...
import torch
from .token_performer import Token_performer_HMSA_decoer_2
...
class decoder_module(nn.Module):
    def __init__(self, dim=384, token_dim=64, img_size=224, ratio=8, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), fuse=True, use_MHSA=False):
        ...
        if self.fuse:
            ...
            # 8_head_self_attention
            if use_MHSA:  # USE MHSA
                self.att = Token_performer_HMSA_decoer_2(dim=token_dim, in_dim=token_dim, kernel_ratio=0.5)
    ...
    def forward(self, dec_fea, enc_fea=None):
        ...
        # [1] token upsampling by the proposed RMT2T module
...
class Decoder(nn.Module):
    def __init__(self, embed_dim=384, token_dim=64, depth=2, img_size=224):
        ...
        self.decoder1 = decoder_module(dim=embed_dim, token_dim=token_dim, img_size=img_size, ratio=8, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), fuse=True, use_MHSA=True)
        self.decoder2 = decoder_module(dim=embed_dim, token_dim=token_dim, img_size=img_size, ratio=4, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), fuse=True, use_MHSA=True)
